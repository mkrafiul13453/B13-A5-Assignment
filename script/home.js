        ...
        card.innerHTML = `
                    <div class="p-4 border-b space-y-3">
                        <div class="flex justify-between items-center mb-3">
                            <div
                                class="w-8 h-8 rounded-full ${issue.status === 'open' ? 'bg-green-700' : 'bg-blue-700'} flex items-center justify-center">
                        ...
                        <div class="flex gap-2 mt-3">
                            ${issue.labels.map(label => `<span
                                class="text-red-500 border border-red-300 bg-red-50 px-3 py-1 rounded-full text-xs font-medium">${label}</span>`).join('')}
                        </div>
                    </div>
...
document.getElementById("btn-search").addEventListener("click", () => {
    ...
    fetch("https://phi-lab-server.vercel.app/api/v1/lab/issues/search?q=notifications")
        ...
        .then((data) => {
            ...
            const filterWord = allTitle.filter((line) => {
                line.title.toLowerCase().includes(searchValue)
            });
